...
# Large 'ALL' PR data isn't loaded yet
# and the results object isn't too big
if (require(pryr))
...
## ----evald---------------------------------------------------------------
# The hapdata object now includes the full PR data
if (require(pryr))
  pryr::object_size(hapdata)
...
## ----snv_boundary, fig.align="center", fig.width=5, fig.height=3, warning=FALSE----
snv_pr <- pr_data(hapdata, var_type = "snv",
                  subset = c("TS_contianed", "TS_boundary"))
ggplot(snv_pr, aes(x = METRIC.Recall, y = METRIC.Precision, col = Subtype)) +
  geom_line() + coord_cartesian(ylim = c(.75, 1)) +
  ggtitle("Longer deletions are more difficult to call accurately")
